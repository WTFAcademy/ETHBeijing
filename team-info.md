...
* 目标：
1. 完成Web前端Demo的开发，为前端Demo开发简单的Node Server，并进行演示。
2. 完成游戏经济模型的设计与开发，结合前端Demo进行铸造展示。
* 是否招募队员：
否，但是黑客松结束之后项目会继续做下去，我们也一直在招募以下岗位的人才：Ui设计师、插画师、产品经理、JAVA后端、React前端、Unity开发等。
* 如何联系：
Wechat ID: Jacob_Mr_Yang
